...
Python port of `packages/coding-agent/src/core/tools/edit-diff.ts`. The
...
def generate_unified_patch(path: str, old_content: str, new_content: str, context_lines: int = 4) -> str:
    r"""Generate a standard unified patch.

    `str.splitlines(keepends=True)` leaves the final line bare when the source
    text has no trailing newline, so `difflib.unified_diff` emits that line
    without a `\n` terminator. Left alone, the next hunk line gets glued onto
    it with no separator, producing a patch a real `patch`/`git apply` rejects
    as malformed. The `diff` npm package's `createTwoFilesPatch` (what the
    TypeScript original uses) instead emits the standard `\ No newline at end
    of file` marker for that line, so it is added here too.
    """
...
def format_os_error(error: OSError) -> str:
    """Format an OS error the way the TypeScript version reads `error.code`/`String(error)`."""
    if error.errno is not None and error.errno in errno.errorcode:
        return f"Error code: {errno.errorcode[error.errno]}"
    return f"Error: {error.strerror or str(error)}"


_error_code_message = format_os_error


def compute_edits_diff(path: str, edits: list[Edit], cwd: str) -> dict:
    """Compute the diff for one or more edit operations without applying them.

    Used for preview rendering before the tool executes. Returns either
    ``{"diff": str, "first_changed_line": int | None}`` or ``{"error": str}``.
    """
    absolute_path = resolve_to_cwd(path, cwd)

    try:
        if not os.path.isfile(absolute_path) or not os.access(absolute_path, os.R_OK):
            # Match the ENOENT/EACCES distinction the TypeScript version gets from `fs.access`.
            if not os.path.exists(absolute_path):
                error = FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT))
            else:
                error = PermissionError(errno.EACCES, os.strerror(errno.EACCES))
            return {"error": f"Could not edit file: {path}. {_error_code_message(error)}."}
        ...
        return {"diff": diff, "first_changed_line": first_changed_line}
    except Exception as err:
        return {"error": str(err)}


def compute_edit_diff(path: str, old_text: str, new_text: str, cwd: str) -> dict:
