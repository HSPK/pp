...
The Python port currently loads prompt templates from:

- Global: `~/.pi/agent/prompts/*.md`
- Project: `.pi/prompts/*.md` (only after the project is trusted)

The loader also has explicit-path support internally, but the current CLI does not wire `--prompt-template` or `settings.json` `prompts` entries into session loading. Python resource packages use a top-level `pi.json` manifest instead of `package.json`'s nested `pi.prompts` field.
...
---
Create a Python class named $1 with features: $@
```
...
- Project templates require project trust.
- Name collisions keep the first loaded template and report a resource diagnostic.
