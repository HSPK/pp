...
Sessions auto-save to `~/.pi/agent/sessions/`, organized by working directory. Each session is an append-only JSONL file with a tree structure.

```bash
pp -c                  # Continue most recent session
pp -r                  # Browse and select from past sessions
pp --no-session        # Ephemeral mode; do not save
pp --name "my task"    # Set session display name at startup
pp --session <path|id> # Use a specific session file or partial session ID
pp --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
| `/export [file]` | Export session. HTML document assembly is not ported; JSONL export is available. |
| `/share` | Attempts secret GitHub gist upload, but shareable HTML assembly is not ported. |

## Resuming and Deleting Sessions

`/resume` opens an interactive session picker for the current project. `pp -r` opens the same picker at startup when a TTY is available.
...
```bash
pp --name "Refactor auth module"
pp --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `pp -r`.
...
Session files are JSONL and contain message entries, model changes, thinking-level changes, labels, compactions, branch summaries, custom entries, session info entries, and custom message entries.
